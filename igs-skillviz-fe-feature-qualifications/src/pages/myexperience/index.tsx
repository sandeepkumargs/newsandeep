...
const tableHeads = ["Company Name", "Designation", "Domain", "Total", "Edit", "Delete"];
      ...
      <section className="flex w-fit flex-col gap-4 rounded-md bg-white p-4">
        <div className="flex justify-between gap-2">
          <div className="flex items-center gap-2">
            <IconEdit />
            <h2 className="text-2xl">Update Your Experience</h2>
          </div>
          <button
            type="button"
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {experiences?.map((experience, idx) => (
                  <tr key={experience.company_name}>
                    ...
                    <td className="px-6 py-3 text-center text-sm capitalize text-gray-600 dark:text-gray-500">
                      {moment.duration(moment(experience.end_date).diff(experience.start_date)).years()
                        ? `${moment.duration(moment(experience.end_date).diff(experience.start_date)).years()} years `
                        : null}{" "}
                      {moment.duration(moment(experience.end_date).diff(experience.start_date)).months()
                        ? `${moment.duration(moment(experience.end_date).diff(experience.start_date)).months()} years `
                        : null}{" "}
                      {`${moment.duration(moment(experience.end_date).diff(experience.start_date)).days()} days`}
                    </td>
                    <td className="px-6 py-3 text-center text-sm capitalize text-gray-600 dark:text-gray-500">
                      <button
                        type="button"
